...
from typing import ClassVar, DefaultDict, List, NamedTuple, Optional, Tuple, Union
...
from overtrack_cv.games.apex.data import rounds, get_round_state, MapLocations
from overtrack_cv.games.apex.processors.minimap import Circle as FrameCircle, Location as FrameLocation, Minimap
from overtrack_cv.games.apex.processors.minimap.models import RingsComposite
...
class Route:
    ...
    def __init__(self, frames: List[Frame], weapons: Weapons, combat: Combat, season: int, debug: Union[bool, str] = False):
        ...
        for i, frame in enumerate([f for f in frames if f.apex.minimap]):
            if 'game_time' in frame:
                final_game_time = frame.game_time

            coordinates = frame.apex.minimap.location.coordinates
            if season <= 2:
                coordinates = (
            ...
            thresh = 0.55
            if frame.apex.minimap and frame.apex.minimap.version == 1:
                thresh = 0.2
            if frame.apex.minimap.location.match > thresh:
                ignored += 1
                continue

            thresh_dist = 100
            if frame.apex.minimap and frame.game_time < 80:
                # dropship speed
                thresh_dist = 500
            elif frame.apex.minimap.location.match < 0.1 and frame.apex.minimap and frame.apex.minimap.version > 0:
            ...
            if len(recent):
                ...
                if not alive_at[int(rts / 10)]:
                    ...
                    if frame.game_time:
                        self._add_circles(circles, frame.game_time, frame.apex.minimap)
                    ...
                    self.logger.warning(
                        f'Ignoring location {i} @{frame.game_time if "game_time" in frame else 0:.1f}s: {frame.apex.minimap.location} - {dist:.1f} away from average {np.round(last, 1)}'
                    )
        ...
        for f in frames:
            if f.apex.minimap and f.apex.minimap.rings_composite:
                rings_composite = f.apex.minimap.rings_composite
                break
        if rings_composite:
    ...
    def _debug(self, frames):
        ...
        plt.plot([f.location.match for f in frames if f.apex.location] + [f.apex.minimap.location.match for f in frames if f.apex.minimap])

        import cv2
        from colorsys import hsv_to_rgb
        from overtrack_cv.games.apex.processors.minimap.minimap_processor import MinimapProcessor

        image = cv2.cvtColor(MinimapProcessor().MAP, cv2.COLOR_GRAY2BGR)
        for frame in [f for f in frames if f.apex.location]:
        ...
        x = np.array([f.apex.minimap.location.coordinates[0] for f in frames if f.apex.minimap])
        y = np.array([f.apex.minimap.location.coordinates[1] for f in frames if f.apex.minimap])
        t = [f.apex.minimap.location.match for f in frames if f.apex.minimap]
