...
class DataclassPrettyPrinter(PrettyPrinter):
    ...
    def _format_dataclass_fields(self, object, object_fields: List[Field], stream, indent, allowance, context, level):
        write = stream.write
        indent += self._indent_per_level
        write('\n' + ' ' * indent)
        delimnl = ',\n' + ' ' * indent
        ...
        for i, field in enumerate(object_fields):
            ...
            write(field.name)
            write('=')
            self._format(getattr(object, field.name), stream, indent + len(field.name) + 1, allowance if last else 1, context, level)
            if not last:
                write(delimnl)
        indent -= self._indent_per_level
        write('\n' + ' ' * indent)
